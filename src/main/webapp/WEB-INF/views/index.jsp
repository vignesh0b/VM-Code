<!DOCTYPE html>
    ...
    <head>
        ...
        <style>
            ...
            * {
            ...
            #save-title-input:focus, #ai-prompt-input:focus {
    ...
    <div id="top-bar">
        <div style="display: flex; align-items: center; gap: 1.5rem;">
            <a href="${pageContext.request.contextPath}/" class="brand" title="Go Home">
            ...
            <button class="ai-btn" onclick="openPromptModal()" title="Ask AI Assistant">
    ...
    <div id="register-modal" class="modal-overlay">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Register an account to save your code and access history.</p>
                <form action="${pageContext.request.contextPath}/register" method="post" id="guest-register-form">
                    <input type="email" name="email" placeholder="Email" required style="width: 100%; padding: 0.8rem 1rem; margin-bottom: 1rem; border-radius: 8px; border: 1px solid var(--border-color); background: rgba(255,255,255,0.05); color: var(--text-primary); outline: none; font-family: var(--font-family);" />
                    <input type="text" name="username" placeholder="Username" required style="width: 100%; padding: 0.8rem 1rem; margin-bottom: 1rem; border-radius: 8px; border: 1px solid var(--border-color); background: rgba(255,255,255,0.05); color: var(--text-primary); outline: none; font-family: var(--font-family);" />
                    <input type="password" name="password" placeholder="Password" required style="width: 100%; padding: 0.8rem 1rem; margin-bottom: 1rem; border-radius: 8px; border: 1px solid var(--border-color); background: rgba(255,255,255,0.05); color: var(--text-primary); outline: none; font-family: var(--font-family);" />
                </form>
            </div>
            <div class="modal-footer">
                <button class="btn-cancel" onclick="closeRegisterModal()">Cancel</button>
                <button class="btn-confirm" onclick="document.getElementById('guest-register-form').submit()">Register</button>
            </div>
        </div>
    </div>
    ...
    <div id="prompt-modal" class="modal-overlay">
        <div class="modal-content" style="max-width: 500px;">
            <div class="modal-header">
                ...
                <button class="close-btn" onclick="closePromptModal()">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                        <line x1="18" y1="6" x2="6" y2="18"></line>
                        <line x1="6" y1="6" x2="18" y2="18"></line>
                    </svg>
            ...
            <div class="modal-body">
                <p>Enter your prompt. The current code will be included as context.</p>
                <textarea id="ai-prompt-input" placeholder="e.g., How can I optimize this algorithm?" rows="4" style="width: 100%; background: rgba(255, 255, 255, 0.05); border: 1px solid var(--border-color); color: var(--text-primary); padding: 0.8rem 1rem; border-radius: 8px; font-family: var(--font-family); font-size: 1rem; outline: none; transition: border-color 0.3s ease; resize: vertical;"></textarea>
            </div>
            <div class="modal-footer">
                <button class="btn-cancel" onclick="closePromptModal()">Cancel</button>
                <button class="btn-confirm" onclick="submitAIPrompt()">Ask AI</button>
            </div>
        </div>
    </div>

    <script>
    const isGuest = ${isGuest != null ? isGuest : false};
    let socket;
    ...
    function runCode() {
        ...
        socket = new WebSocket("ws://" + window.location.host + "${pageContext.request.contextPath}/ws");
    ...
    async function explainCode() {
        ...
        try {
            const res = await fetch("${pageContext.request.contextPath}/ai/explain", {
                method: "POST",
                headers: {
    ...
    async function debugCodeAI() {
        ...
        try {
            const res = await fetch("${pageContext.request.contextPath}/ai/debug", {
                method: "POST",
                headers: {
                    "Content-Type": "text/plain"
                },
    ...
    async function submitAIPrompt() {
        ...
        try {
            const res = await fetch("${pageContext.request.contextPath}/ai/prompt", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json"
                },
                body: JSON.stringify({ code: code, prompt: promptInput })
            });
            const text = await res.text();
            updateTerminal(text);
        } catch (err) {
    ...
    async function confirmSaveCode() {
        ...
        try {
            const res = await fetch("${pageContext.request.contextPath}/api/save", {
                method: "POST",
    ...
    async function openHistoryModal() {
        ...
        try {
            const res = await fetch('${pageContext.request.contextPath}/api/history');
            if (res.status === 401 && typeof isGuest !== 'undefined' && isGuest) {
                closeHistoryModal();
                document.getElementById('register-modal').classList.add('active');
                return;
            }
